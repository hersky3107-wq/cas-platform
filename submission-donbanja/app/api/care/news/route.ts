...
import { askPerplexity } from '@/lib/care/resident-search'

/**
 * 오늘의 소식 — elderly-friendly daily news for the 동반자 (care) app.
 *
 * Cache table: `care_news`.
 *
 ...
 * Slow path: Perplexity (national) + Perplexity (user's region) → Claude rewrite
 * into plain, elderly-friendly Korean JSON → upsert on the unique `slot`.
 *
...
function buildRewriteSystem(regionLabel: string): string {
  return `당신은 어르신을 위한 라디오 뉴스 진행자입니다. 아래에 오늘의 전국 뉴스와 ${regionLabel} 지역 뉴스 검색 결과가 주어집니다. 이것을 어르신이 듣기 쉬운 아주 쉬운 한국어로 다시 써서 순수 JSON으로만 출력하세요. 마크다운 없이.
...
5. 기타
   ...
   - ${regionLabel} 지역 소식은 전국보다 더 두껍게 5~6개 이상.

출력 JSON 스키마(정확히 이 형태 — "region_news" 키는 위 지역 소식을 담는 칸입니다):
{
  "national": [ { "section": "정치"|"경제"|"사회"|"국제"|"문화·예술"|"스포츠", "title": "짧은 제목", "summary": "2~3줄 쉬운 요약" } ],
  "region_news": [ { "title": "짧은 제목", "summary": "2~3줄 쉬운 요약" } ]
}
JSON 객체만 출력하세요.`
}

async function rewriteToPlain(nationalText: string, localText: string, regionLabel: string, apiKey: string): Promise<{ national: NationalItem[]; region_news: RegionNewsItem[] } | null> {
  const user = `[전국 뉴스 검색 결과]\n"""\n${nationalText.slice(0, 6000)}\n"""\n\n[${regionLabel} 지역 뉴스 검색 결과]\n"""\n${localText.slice(0, 6000)}\n"""`
  const raw = await callClaude(buildRewriteSystem(regionLabel), user, apiKey, 3500)
  ...
  try {
    ...
    const national: NationalItem[] = Array.isArray(parsed.national)
      ...
      : []
    const region_news: RegionNewsItem[] = Array.isArray(parsed.region_news)
      ? (parsed.region_news as unknown[])
          .filter((x): x is Record<string, unknown> => !!x && typeof x === 'object')
          .map((x) => ({
            title: typeof x.title === 'string' ? x.title.trim() : '',
            summary: typeof x.summary === 'string' ? x.summary.trim() : '',
          ...
          .filter((it) => it.title.length > 0 && it.summary.length > 0)
      : []
    if (national.length === 0 && region_news.length === 0) return null
    return { national, region_news }
  } catch {
    return null
  }
}
...
export async function GET(req: Request) {
  const { slot: baseSlot, freshLabel } = computeSlot(new Date())

  // Residence context (from the client query string). The regional section is
  // "우리 지역 소식" for the user's residence; national news stays national.
  const url = new URL(req.url)
  const sido = (url.searchParams.get('sido') ?? '').trim()
  const sigungu = (url.searchParams.get('sigungu') ?? '').trim()
  const sidoCode = (url.searchParams.get('sidoCode') ?? '').trim()
  const regionLabel = sigungu && sido ? `${sido} ${sigungu}` : sido || '우리 지역'
  // Region-specific cache key so each residence caches separately.
  const regionSuffix = /^\d{6}$/.test(sidoCode) ? sidoCode : 'na'
  const slot = `${baseSlot}-${regionSuffix}`

  // 1) Fast path — cache lookup
  try {
    const { data, error } = await supabaseAdmin
      .from('care_news')
      .select('national, region_news, sources, generated_at')
      .eq('slot', slot)
      .maybeSingle()
  ...
  try {
    ...
    // Perplexity 2 — the user's regional news (thicker, life-relevant)
    const localRes = await askPerplexity(
      `오늘(${dateLabel}) ${regionLabel} 지역에서 실제로 일어난 주요 지역 소식을 알려줘. 그 지역 주민의 일상생활에 중요한 생활·행정·복지·수당·날씨·행사·교통·안전·건강 관련 소식을 6개 이상 구체적으로 정리해줘. 실제 결정·시행·발표된 사실 위주로, 전망이나 분석 기사는 제외.`,
      {
        systemPrompt:
          `당신은 오늘의 ${regionLabel} 지역 소식을 정리하는 검색 도우미입니다. 실제 오늘·최근 보도된 해당 지역 소식만, 사실 위주로 정리하세요. 주민 생활(복지·안전·교통·행사)에 영향 있는 소식을 우선하세요. 추측하지 말고 확인된 내용만.`,
        maxTokens: 1200,
      }
    )

    const nationalText = nationalRes.text
    const localText = localRes.text
    if (!nationalText.trim() && !localText.trim()) {
      return gracefulError()
    }

    // Claude rewrite → plain elderly-friendly JSON
    const rewritten = await rewriteToPlain(nationalText, localText, regionLabel, apiKey)
    if (!rewritten) {
      return gracefulError()
    }

    const sources = Array.from(new Set([...nationalRes.citations, ...localRes.citations])).slice(0, 12)
    const generatedAt = new Date().toISOString()

    const payload: NewsPayload = { national: rewritten.national, region_news: rewritten.region_news, sources }
        ...
        .upsert(
          {
            ...
            region_news: payload.region_news,
