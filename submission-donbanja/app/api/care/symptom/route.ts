 ...
 *  3. Perplexity finds SPECIFIC clinics in the user's region for that 진료과,
 *     prioritizing 동네 의원(1차 의료기관), excluding big general hospitals for
 *     minor symptoms.
 *  4. Claude extracts clinic names/areas/tier from the Perplexity text.
 *  5. Cross-reference each clinic against the HIRA list for that 시·도 to get
 *     VERIFIED phone/address/type. If not found in HIRA → show name + area only
 *     (no invented phone), marked source='perplexity'.
...
export async function POST(req: Request) {
  ...
  // 3) Perplexity — find specific clinics in the user's region (clinic-first)
