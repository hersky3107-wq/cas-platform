...
/**
 * 지금 문 연 곳 — open-now hospitals & pharmacies for the user's residence (stateless).
 *
 * Input:  POST { kind: '병원' | '약국', area?: string, sidoCd?: string, regionLabel?: string }
 *
 * Flow:
...
const EXTRACT_SYSTEM = `아래 검색 결과 텍스트에서 해당 지역에서 지금 운영 중이거나 야간·공휴일에 운영하는 병원·의원·약국의 이름, 위치, 운영시간 메모를 추출해 순수 JSON 배열로만 출력하세요. 마크다운 없이.
...
export async function POST(req: Request) {
  ...
  const areaHint = area ? `${area} 및 인근` : `${regionLabel} 전역`
  const kindHint =
    kind === '약국'
  ...
  const { text: pplxText, citations } = await askPerplexity(
    `현재 시각 ${nowKST} 기준으로 ${areaHint}에서 지금 문 열어 있는 ${kind}을 알려줘. 특히 ${kindHint}을 우선해서 이름, 위치(시·군·구·동), 운영시간 정보를 구체적으로 알려줘.`,
    {
      systemPrompt:
        `당신은 ${regionLabel} 의료기관 및 약국 운영 현황을 찾는 검색 도우미입니다. 현재 운영 중이거나 야간·공휴일에 운영하는 곳만, 이름·위치·운영시간 위주로 한국어로 구체적으로 정리하세요. 추측하지 말고 확인된 정보만 제시하세요.`,
      maxTokens: 800,
    }
  )
