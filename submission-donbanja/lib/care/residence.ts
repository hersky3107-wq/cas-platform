...
export interface SidoInfo {
  ...
  code: string
  /** English slug used as the welfare `region` key (e.g. "seoul", "cheju"). */
  regionKey: string
 ...
 * (e.g. 서울→'seoul', 제주→'cheju'); matchWelfare then reads region IN (key,
 * 'national'), so the user sees their province's local-government welfare plus
 * nationwide central-government welfare.
 */
 ...
 * care_welfare_services table. Returns null when the name is unknown.
 *
