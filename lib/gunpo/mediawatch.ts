...
/**
 * Today's KST calendar date as "YYYY년 M월 D일" (no leading zeros on M/D).
 * Used in the hard 5-day recency instruction so the model sees an unambiguous
 * Korean date — never hardcoded.
 */
function todayKstYmdKo(): string {
...
function buildSearchSystemPrompt(today: string): string {
  const todayYmd = todayKstYmdKo()
  return [
    `당신은 경기도 군포시 관련 최신 언론 보도를 찾아 정리하는 검색 전문가입니다. 오늘은 ${today}(한국 표준시)입니다.`,
    `오늘은 ${todayYmd}이다. 반드시 최근 5일 이내(오늘 포함)에 보도된 기사만 수집하라. 5일 이전 기사는 제외하라. 날짜가 불명확한 기사도 제외하라.`,
    '주어진 주제 영역에 대해, 최근 5일 이내의 실제 군포시·경기도 관련 보도·공고를 찾아 한국어로 정리하세요.',
    '검색 축은 두 가지입니다:',
    ...
    '',
    '【군포 기초언론 직접 방문 — 필수】',
    '다음 군포 지역 기초언론 사이트를 직접 방문해 최근 5일 이내 기사 목록을 확인하라. 검색에 의존하지 말고 각 사이트에 직접 접속해 최신 기사 제목·날짜·요약을 읽어와라:',
    '- 군포시민신문 https://www.mediagunpo.co.kr',
    '- 군포뉴스 https://gunponews.co.kr',
    '- 군포포털/군포타임뉴스',
    '이들 지역지는 대형 포털 검색에 잘 노출되지 않으므로 사이트 직접 방문이 반드시 필요하다. 각 사이트에서 최근 5일 이내(오늘 포함) 기사만 수집하고, 날짜가 확인되지 않으면 제외하라.',
    '',
    '위 지역지 직접 방문과 함께, 경기신문·경기일보·뉴시스 등 경기·전국 매체와 군포시청 공식 발표도 함께 수집하세요. 지역지 직접 방문은 대체재가 아니라 추가 필수 단계입니다.',
    '',
    '목표: 해당 영역에서 실제로 보도된 기사·보고서를 3~5건 찾아 각각 따로 정리하세요. 보도가 실제로 많으면 5건, 적으면 있는 만큼(최소 1건)만 쓰세요. 5일 윈도우 안에서 찾을 수 없으면 억지로 채우지 마세요.',
    '각 보도 항목마다 다음 네 가지를 빠짐없이 포함하세요:',
    '① 언론사(매체명): 군포·경기 지역언론(군포시민신문·군포뉴스·군포타임뉴스·경기일보·경기신문·군포시청 공식 홈페이지 등)이면 "지역지", 전국 언론이면 "전국지"로 함께 표시.',
    '② 헤드라인 요지: 제목을 그대로 베끼지 말고 의미를 풀어서 쓰세요(직접 인용은 아주 짧게만, 큰따옴표로).',
    '③ 보도 날짜: 반드시 구체적으로(가능하면 YYYY-MM-DD) 명시하세요. 날짜를 최근 5일 이내로 확정할 수 없으면 그 기사는 결과에 넣지 마세요 — "날짜 불명"·"오래된 기사"로 표기해 포함하지 마세요.',
    '④ 논조(framing): 이 보도가 사안을 어떻게 바라보는지(긍정적/부정적/중립적, 무엇을 강조하고 무엇을 우려하는지) 1~2문장으로 설명하세요.',
    '가능하면 출처 URL도 포함하세요(특히 지역지 직접 방문 기사는 해당 사이트 URL).',
    '',
    '가능하면 군포 지역지·경기 지역언론과 전국 언론을 섞어서 폭넓게 보세요.',
    '규칙(엄수): 실재하지 않는 기사·날짜·수치를 절대 지어내지 마세요. 지지율·찬성률 같은 정량 수치는 만들지 말고, 보도 논조는 정성적으로만 다루세요. 해당 영역에서 최근 5일 이내 보도를 찾지 못했으면 "이 영역에서 최근 5일 이내 군포시·경기도 관련 보도를 찾지 못했습니다"라고 솔직히 쓰세요.',
  ].join('\n')
}
...
async function runOneMediaSearch(
  ...
  const query = [
    `${umbrella.hint} — 오늘(${todayKstYmdKo()}) 기준 최근 5일 이내 군포시·경기도 관련 보도만.`,
    '반드시 https://www.mediagunpo.co.kr 와 https://gunponews.co.kr 를 직접 방문해 해당 주제의 최근 5일 기사를 확인하고, 경기신문·경기일보·뉴시스 등도 함께 수집하라.',
  ].join(' ')
...
function buildSynthesisSystemPrompt(mode: JejuMediaWatchMode, today: string): string {
  ...
  return [
    ...
    `오늘은 ${todayYmd}이다. 브리핑에는 반드시 최근 5일 이내(오늘 포함)에 보도된 기사만 반영하라. 5일 이전 기사는 제외하라. 날짜가 불명확하거나 최근 5일 이내임을 확인할 수 없는 기사는 "날짜 불명"으로 표기하지 말고 아예 제외하라.`,
    ...
    perspectiveFor(mode),
    '',
    '기간 필터(최우선): 수집 자료에 5일보다 오래된 기사·날짜 불명 기사가 섞여 있어도 요약·핵심·주변에 넣지 마세요. 5일 윈도우 안에서 확인할 수 있는 보도만 쓰세요. 필터 후 자료가 비면 해당 절에 "최근 5일 이내 확인 가능한 보도가 적었습니다"라고 한 줄로 쓰세요.',
    '',
    '분량 지침(중요): A4 1.5~2장 분량의 충실한 한국어 브리핑을 목표로 하세요. 수집된 실제 보도가 풍부하면 그것을 충분히 반영해서 쓰세요. 단, 분량은 반드시 실제 수집 자료(5일 이내)에서 나와야 합니다 — 자료가 빈약한 영역을 억지로 늘리지 마세요.',
    '',
    '출력 형식(매우 중요): 오직 하나의 JSON 객체만 출력하세요. 마크다운 코드펜스(```)도, 설명 문장도 쓰지 마세요. 순수 JSON만. 각 값은 한국어 산문/목록 문자열입니다.',
    '스키마:',
    '{',
    '  "summary": "최상단 요약 (4~6줄). 오늘 군포시·경기도 언론 지형의 큰 그림 — 가장 뜨거운 사안 2~3개와 전체 논조 분위기(최근 5일 이내 보도만).",',
    '  "coreIssues": "핵심 이슈 — 여러 매체에서 크게 다뤄진 주요 사안들(최근 5일 이내만). 각 사안마다: (1) 어떤 매체들이 어떤 날짜에 보도했는지, (2) 각 보도의 논조(긍정/부정/중립, 강조점, 우려), (3) 매체 간 논조 차이가 있으면 대비, (4) 이 사안이 정책적으로 왜 중요한지. 사안이 여럿이면 각각 충분히 다루세요. 이 섹션에 가장 큰 비중을 두세요.",',
    '  "minorIssues": "주변 이슈 — 보도량은 적지만 놓치면 안 될 사안들(최근 5일 이내만). 각 항목마다 매체·날짜·논조를 한 줄~두 줄로. 항목이 여럿이면 모두 포함하세요.",',
