...
import { buildMotieSupplementBlock, type MotieSupplement } from '@/lib/gunpo/supplements'
...
export async function summarizeAvailableData(
  ...
  // TODO(군포): brief.ts의 TRADE_SOURCE_IDS/WARROOM_SOURCE_IDS가 현재 비어있어
  // (gatherJejuSnapshot이 항상 sources:[] 반환) 아래 맵은 당장은 참조되지 않는
  // 죽은 코드다. 소스 id를 채우는 단계에서 이 맵도 lib/gunpo/connectors.ts의
  // 실제 id(kamis-gunpo-products 등)에 맞춰 다시 쓸 것 — 지금은 motie 원본
  // 그대로 남겨둔 참고용 예시일 뿐이다.
...
function buildTradeChairSystemPrompt(consensusScore: number, brief: boolean, hasVote: boolean): string {
  const lines = [...TRADE_CHAIR_PERSONA_LINES, GUNPO_DELIBERATION_DIRECTIVES]
