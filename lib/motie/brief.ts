...
import { fetchJejuSource } from '@/lib/motie/connectors'
import {
  runSingleAiProvider,
  MODEL_BY_PROVIDER,
...
/**
 * AX COUNCIL top-level mode (STEP 2A). Branches the data layer:
 *   - 'trade'   (수출참모): KOTRA 국가정보 + 상품DB + 환율 (TRADE_SOURCE_IDS).
 *   - 'warroom' (자원·에너지 워룸): STUB — still uses the governance sources until
 *     STEP 2C wires oil/gas/energy-security connectors (kept intact for now).
 */
export type JejuCouncilMode = 'trade' | 'warroom'

/** The governance-axis sources: energy, agriculture/marine, climate, forecast. */
 ...
 * 'warroom' → 오피넷 유가 + 가스공사 LNG (자원·에너지 워룸).
 */
...
const SECTION_HEADER_BY_ID: Record<string, string> = {
  ...
  'kotra-nation-info': '## KOTRA 국가정보 (주요 수출시장)',
  'kotra-product-db': '## KOTRA 상품DB (국가별 유망상품)',
...
/**
 * Standing energy-security context for WARROOM mode (자원·에너지 워룸). Injected into
 * every warroom briefing so the panel treats fuel prices + LNG imports as parts of
 * a single import-dependent energy-security picture. Public APIs cover national fuel
 * prices (오피넷) and continent-level LNG imports (가스공사); international 정세·원유
 * 지표는 검색(Perplexity)으로 보완할 것.
 */
export const WARROOM_STANDING_ENERGY_CONTEXT =
  '자원·에너지 안보 맥락(상시): 한국은 에너지의 대부분을 수입에 의존한다(원유·LNG 등). ① 국내 유가는 국제 유가·환율·유류세에 연동되며, 전국 평균 주유소 판매가(오피넷)로 소비자 체감 물가와 산업 비용을 파악한다. ② LNG는 대륙별 수입처(중동·아시아·오세아니아 등) 구성과 단위가격이 에너지 안보·전력 원가에 직결된다. ③ 국제 원유(WTI·Brent)·지정학 리스크·주요 산유국 동향 등 공공API로 커버되지 않는 지표는 검색(Perplexity)으로 최신 정세를 확인할 것.'
