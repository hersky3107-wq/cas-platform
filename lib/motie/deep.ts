 ...
 */
export async function summarizeAvailableData(
  ...
  return councilMode === 'trade' ? dataLines : `${WARROOM_STANDING_ENERGY_CONTEXT}\n\n${dataLines}`
}
...
export async function planJejuMeeting(params: {
  ...
  try {
    r = await runSingleAiProvider({
      ...
      prompt: userPrompt,
      systemPrompt: buildOrchestratorSystemPrompt(debateBrands, councilMode),
...
function buildAnalystSystemPrompt(
  ...
  const lines = [
    analystPersonaLine(councilMode, role.roleLabel),
  ...
  if (isTrade) {
    lines.push(TRADE_REDUNDANCY_RULE, TRADE_ANALYST_DIRECTIVE)
  } else {
    lines.push(WARROOM_REDUNDANCY_RULE, WARROOM_ANALYST_DIRECTIVE)
  }
  ...
  lines.push('', `[${systemAgendaLabel(councilMode)}] ${question}`)
...
export async function runExpertAnalyses(params: {
  ...
  const settled = await Promise.allSettled(
    roles.map((role) => runOneExpert(role, question, context, councilMode))
  )
...
export async function runJejuDeepThroughAnalysis(params?: {
  ...
  try {
    snapshot = await gatherJejuSnapshot(councilMode)
    context = buildBriefingContext(snapshot, councilMode)
    ...
    const analyses = await runExpertAnalyses({ question, roles: plan.roles, context, councilMode })
...
export async function mergeSearchRequests(params: {
  ...
  try {
    r = await runSingleAiProvider({
      ...
      provider: 'anthropic',
      prompt: userPrompt,
      systemPrompt: buildMergeSystemPrompt(councilMode),
...
async function runOneSearch(
  ...
  try {
    r = await runSingleAiProvider({
      ...
      provider: 'perplexity',
      prompt: item.query,
      systemPrompt: buildSearchSystemPrompt(councilMode),
...
export async function executeJejuSearches(params: {
  ...
  const settled = await Promise.allSettled(toRun.map((item) => runOneSearch(item, councilMode)))
...
export async function reviseExpertAnalyses(params: {
  ...
  const settled = await Promise.allSettled(
    analyses.map((a) => {
      ...
      return runOneRevision(a, role, question, sharedSearchBlock, councilMode)
    })
  )
...
function buildDebateSystemPrompt(
  ...
  if (councilMode === 'trade') {
    lines.push(TRADE_ANALYST_DIRECTIVE)
  } else {
    lines.push(WARROOM_ANALYST_DIRECTIVE)
  }
...
export async function runDebateRound(params: {
  ...
  const settled = await Promise.allSettled(
    revised.map((self) => {
      ...
      return runOneRebuttal(self, role, question, peerBlock, councilMode)
    })
  )
...
async function measureConsensus(
  ...
  try {
    r = await runSingleAiProvider({
      ...
      provider: 'anthropic',
      prompt: userPrompt,
      systemPrompt: buildConsensusSystemPrompt(councilMode),
...
export async function runDeliberationRound(params: {
      ...
      const ownPrior =
        prior && prior.ok && prior.position
          ...
          : seed && seed.ok && seed.revised
            ? seed.revised
            : ''
      return runOneDeliberationTurn(role, question, roundNumber, ownPrior, peerContext, councilMode)
    })
  )
  ...
  const consensus = await measureConsensus(turns, councilMode)
...
/**
 * Default disclaimer appended when the chair omits its own 참고 사항 section.
 * lib/motie defaults to warroom (자원·에너지 안보); trade/verdict paths override via
 * disclaimerFor(councilMode), so this is only a last-resort fallback.
 */
const DEFAULT_DISCLAIMER = disclaimerFor('warroom')

/**
 * Strengthened 소수의견(마이너리티 리포트) instruction — a key differentiator: dissent
 * is preserved substantively, not as a formality. Requires (1) who dissented + why,
 * (2) a steelmanned strongest counter-argument, (3) what evidence/condition would flip
 * the majority; ≥3–4 sentences; never empty even on near-unanimous votes.
 */
const MINORITY_REPORT_INSTRUCTION_TRADE =
  '이 절은 형식적으로 채우지 말고 실질적으로 쓰십시오. 반드시 (1) 어느 관점·부처(좌석)가 왜 이견을 냈는지 명시하고, (2) 그 반대 측이 제시한 가장 강력한 논거를 스틸맨(steelman)하여 공정하게 서술하며(무시·폄하 금지), (3) 어떤 근거·조건이 확인되면 다수 판단이 바뀔 수 있는지 적으십시오. 최소 3~4문장 이상으로 쓰십시오. 표결이 거의 만장일치였더라도 "이견 없음"으로 비우지 말고, 토론 중 제기된 가장 중요한 유보·리스크 하나를 반드시 짚어 기업이 놓치지 않게 하십시오. 제목은 그대로 두고 내용은 한국어로 쓰십시오.'

const MINORITY_REPORT_INSTRUCTION_WARROOM =
  '이 절은 형식적으로 채우지 말고 실질적으로 쓰십시오. 반드시 (1) 어느 관점·부처(좌석)가 왜 이견을 냈는지 명시하고, (2) 그 반대 측이 제시한 가장 강력한 논거를 스틸맨(steelman)하여 공정하게 서술하며(무시·폄하 금지), (3) 어떤 근거·조건이 확인되면 다수 판단이 바뀔 수 있는지 적으십시오. 최소 3~4문장 이상으로 쓰십시오. 표결이 거의 만장일치였더라도 "이견 없음"으로 비우지 말고, 토론 중 제기된 가장 중요한 유보·리스크 하나를 반드시 짚어 정책결정자가 놓치지 않게 하십시오. 제목은 그대로 두고 내용은 한국어로 쓰십시오.'

/** The chair's final, structured verdict — the deliverable officials read. */
export type JejuVerdict = {
  ok: boolean
  /** 3-line summary of the KEY CONTESTED AXES, shown ABOVE the verdict so a judge grasps the debate at a glance. */
 ...
 */
function buildTradeChairSystemPrompt(consensusScore: number, brief: boolean, hasVote: boolean): string {
  const lines = [...TRADE_CHAIR_PERSONA_LINES]

  if (consensusScore >= CONSENSUS_TARGET) {
    lines.push(
      `참모들의 합의도는 ${consensusScore}점으로 높습니다. 대체로 합의했으므로, 그 합의를 확정하고 날카롭게 다듬으십시오.`
    )
  } else if (consensusScore < 70 && consensusScore >= 0) {
    lines.push(
      `참모들이 완전히 합의하지 못했습니다(합의도 ${consensusScore}점, 70점 미만). 바로 이럴 때 당신의 판단이 가장 중요합니다. "참모들이 갈렸다"로 회피하지 말고, 흩어진 근거를 저울질하여 당신이 방어할 수 있는 가장 타당한 단일 결론(진입·보류·조건부 추진 등)을 책임지고 내리십시오. 기업이 곧바로 실행할 수 있는 분명한 방향을 제시해야 합니다.`
    )
  } else {
    lines.push(
      `참모들이 완전히 합의하지 못했습니다(합의도 ${consensusScore === CONSENSUS_SCORE_UNAVAILABLE ? '측정 불가' : consensusScore + '점'}). 근거를 들어 가장 타당한 결론을 책임지고 내리십시오.`
    )
  }
  ...
  if (hasVote) {
    lines.push(
      '',
      '이 안건에는 참모 패널 전원의 표결 결과가 참고 자료로 함께 제공됩니다.',
      '표결 결과는 참고 자료일 뿐, 절대적 구속력이 없습니다. 다수결을 맹종하지 말고, 표결과 다른 판단을 내릴 수 있으며 그럴 경우 그 이유를 "## 최종 판단"에 분명히 밝히십시오.',
      '표결은 4가지입니다(찬성 / 조건부 찬성 / 기권 / 반대). "조건부 찬성"이 많다는 것은 방향에는 합의하되 세부 조건·전제가 필요하다는 뜻이므로, 이를 반대나 불합의로 읽지 마십시오 — 오히려 "방향 합의 + 조건부 추진"으로 판단하고, 표결 이유에 제시된 조건들을 최종 판단의 실행 조건으로 통합하십시오.'
    )
  }

  // Fact-vs-estimate + import-regulation + product-category + FX discipline.
  lines.push('', TRADE_ANALYST_DIRECTIVE)

  if (brief) {
    lines.push(
      '',
      '참모 합의도가 높아 길게 판결할 필요가 없습니다. 아래 구조를 정확히 따르되 간결하게 작성하십시오. 각 절은 반드시 "## " 머리말로 시작하세요. 수집 데이터 요약과 참모 분석·조사 요약 절은 생략합니다.',
      '중요: 분량을 줄이더라도 "## 소수의견 (마이너리티 리포트)"는 절대 생략하지 마십시오.',
      '',
      '## 핵심 쟁점 (3줄 요약)',
      '출력을 반드시 이 절로 시작하십시오. 서론·머리말 없이, 이 심의에서 의견이 갈린 핵심 축을 정확히 3줄(각 줄 "- "로 시작하는 한 문장)로 요약하십시오. 예: 시장 기회 vs 규제·인증 장벽, 조기 진입 속도 vs 리스크 신중론, 단기 매출 vs 장기 브랜드. 3줄을 넘기지 마십시오.',
      '',
      '## 최종 판단',
      '당신의 결론(진입/보류/조건부 추진 등)과 핵심 이유를 간결하게. 기업이 실행할 수 있는 분명한 방향으로 쓰되, 추정·전망에는 [AI 추정]을 붙이십시오.',
      '',
      '## 토론·합의 과정',
      '참모 논의가 어떻게 수렴했는지 짧게(합의 점수와 안착 지점 위주).',
      '',
      '## 소수의견 (마이너리티 리포트)',
      MINORITY_REPORT_INSTRUCTION_TRADE,
      '',
      '## 참고 사항',
      TRADE_DISCLAIMER,
      '',
      '데이터 정직성: 반드시 제공된 심의 자료에 근거하십시오. 공식 데이터와 [AI 추정]을 구분하고, 자료에 없는 사실을 지어내지 마십시오. 근거 없는 구체 수치(예산 규모·기간·금액)는 판결에서 반복하지 말고 정성적으로 다루십시오.'
    )
    return lines.join('\n')
  }
  ...
  lines.push(
    ...
    '분량 배분(중요): "## 최종 판단"에 가장 큰 비중을 두되, 요약 절은 각각 핵심만 간결하게 쓰고, 반드시 마지막 "## 소수의견 (마이너리티 리포트)"와 "## 참고 사항"까지 빠짐없이 작성하십시오.',
    '',
    '## 핵심 쟁점 (3줄 요약)',
    '출력을 반드시 이 절로 시작하십시오. 서론·머리말 없이, 이 심의에서 의견이 갈린 핵심 축을 정확히 3줄(각 줄 "- "로 시작하는 한 문장)로 요약하십시오. 예: 시장 기회 vs 규제·인증 장벽, 조기 진입 속도 vs 리스크 신중론, 단기 매출 vs 장기 브랜드. 3줄을 넘기지 마십시오.',
    '',
    ...
    '',
    '## 토론·합의 과정',
    '논의가 어떻게 수렴했는지 제대로 요약(합의 점수, 어디에 왜 안착했는지).',
    '',
    '## 현지 리스크',
    '심의 자료에 현지 시장·언론·경쟁 분석이 포함된 경우에만 이 절을 작성하라. 진입 시 예상되는 현지 시장·규제·평판 리스크를 정성적으로 서술하라. 정량 수치를 지어내지 말 것. 관련 분석이 전혀 없으면 이 절을 생략하라.',
    '',
    '## 소수의견 (마이너리티 리포트)',
    MINORITY_REPORT_INSTRUCTION_TRADE,
    '',
...
function buildChairSystemPrompt(
  consensusScore: number,
  brief = false,
  hasVote = false,
  councilMode: JejuCouncilMode = 'warroom'
  ...
  if (councilMode === 'trade') {
    return buildTradeChairSystemPrompt(consensusScore, brief, hasVote)
  }
  const lines = [
    ...WARROOM_CHAIR_PERSONA_LINES,
    '',
  ...
  if (consensusScore >= CONSENSUS_TARGET) {
    ...
    lines.push(
      `전문가들이 완전히 합의하지 못했습니다(합의도 ${consensusScore}점, 70점 미만). 바로 이럴 때 당신의 판단이 가장 중요합니다. "전문가들이 갈렸다"로 회피하는 것은 직무 유기입니다. 흩어진 근거들을 저울질하여, 당신이 방어할 수 있는 가장 타당하고 확실한 단일 결론을 책임지고 판결하십시오. 모호한 양비론이 아니라, 정책결정자가 곧바로 대응할 수 있는 분명한 방향을 제시해야 합니다.`
    )
  } else {
    lines.push(
  ...
  if (brief) {
    ...
    lines.push(
      ...
      '중요: 분량을 줄이더라도 "## 소수의견 (마이너리티 리포트)"는 절대 생략하지 마십시오. 합의도가 높아도 끝까지 남은 소수·반대 의견은 반드시 보존해야 합니다.',
      '',
      '## 핵심 쟁점 (3줄 요약)',
      ...
      '',
      '## 소수의견 (마이너리티 리포트)',
      MINORITY_REPORT_INSTRUCTION_WARROOM,
      '',
      '## 참고 사항',
      WARROOM_DISCLAIMER,
      '',
  ...
  lines.push(
    '',
    '아래 구조를 정확히 따르되, 각 절은 한국어 산문으로 작성하십시오. 각 절은 반드시 "## " 머리말로 시작하세요.',
    '분량 배분(중요): "## 최종 판단"에 가장 큰 비중을 두되, 요약 절(수집 데이터/전문가 분석·조사/토론·합의 과정)은 각각 핵심만 간결하게 쓰고, 반드시 마지막 "## 소수의견 (마이너리티 리포트)"와 "## 참고 사항"까지 빠짐없이 작성하십시오. 중간에서 분량이 소진되어 소수의견이 누락되는 일이 없도록 하십시오.',
    '',
    '## 핵심 쟁점 (3줄 요약)',
    '출력을 반드시 이 절로 시작하십시오. 서론·머리말 없이, 이 심의에서 의견이 갈린 핵심 축을 정확히 3줄(각 줄 "- "로 시작하는 한 문장)로 요약하십시오. 예: 예산 vs 규제 우선순위, 추진 속도 vs 신중론, 단기 효과 vs 장기 리스크. 3줄을 넘기지 마십시오.',
    '',
...
export async function renderChairVerdict(params: {
  ...
  const base = {
    keyIssues: null as string | null,
    consensusScore,
    disclaimer: disclaimerFor(councilMode),
    provider: 'anthropic',
  }
  ...
  const contextParts = [
    ...
    '',
    '# 1. 수집 데이터 (beat 1)',
    buildBriefingContext(snapshot, councilMode),
    '',
  ...
  try {
    r = await runSingleAiProvider({
      ...
      provider: 'anthropic',
      prompt: contextBlock,
      systemPrompt: buildChairSystemPrompt(consensusScore, brief, hasVote, councilMode),
  ...
  return {
    ...
    minorityReport,
    // Prefer the chair's own 참고 사항 if present; otherwise the mode default.
    disclaimer: parsed.disclaimer ?? disclaimerFor(councilMode),
  }
}
...
export type JejuVoteResult = {
  ...
  /** approved if (approve+conditional)>oppose; rejected if oppose>(approve+conditional); divided if equal. 조건부 찬성 is directional agreement so it counts on the yes side; abstains never tip it. */
  outcome: 'approved' | 'rejected' | 'divided'
  ok: boolean
  /** Korean one-liner, e.g. '찬성 4 · 조건부 찬성 2 · 기권 1 · 반대 1 — 다수 승인'. */
  summary: string
}
...
function buildVoteSystemPrompt(mode: JejuVoteMode = 'verdict', councilMode: JejuCouncilMode = 'warroom'): string {
  ...
  const stancePrimary = [
    ...
    '  2) 토론 내내 안건의 핵심 방향·수단을 조건 없이 지지했다면 → 찬성.',
    '  3) 큰 방향·핵심 수단에는 동의하나 세부 조건·전제(시점·순서·규모·보완장치)가 반드시 필요하다면 → 조건부 찬성. 그 조건을 이유에 명시하십시오.',
    ...
    '  부차적 우려는 조건부 찬성의 사유입니다(조건을 이유에 적고 "조건부 찬성"을 선택). 핵심 방향·수단을 지지했다면 반대로 표결하는 것은 금지됩니다.',
    ...
    '',
    '찬성·조건부 찬성·기권·반대의 정의:',
    '  찬성: 안건이 제시한 그 수단·방향이 옳고, 별도의 전제 조건 없이 지금 그대로 추진해도 좋다.',
    '  조건부 찬성: 큰 방향·핵심 수단에는 동의하나, 시행 시점·순서·규모·보완장치 등 세부 조건·전제가 충족되어야 한다 → 이 옵션을 선택하고 그 조건을 이유에 반드시 명시하십시오. (찬성이나 기권으로 뭉뚱그리지 마십시오.)',
    '  반대: 그 수단·방향 자체가 틀렸거나, 다른 수단을 택해야 한다.',
    '  기권: 전문 영역 밖이거나 근거가 부족해 진정으로 판단할 수 없다.',
    '',
    '조건부 찬성과 방향성 반대의 구분:',
    '  조건부 찬성: 핵심 수단·기제는 받아들이되 시행 시점·순서·규모·전제 조건만 더 필요하다고 보면 → "조건부 찬성"을 선택하고 이유에 조건을 적으십시오.',
