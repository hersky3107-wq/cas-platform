...
export function openOrchestratorPersonaLines(m: JejuCouncilMode): string[] {
  return isTrade(m)
    ? [
        ...
        '6개 추론 AI를 각기 다른 중립적 분석 렌즈(시장·수요 / 관세·인증·규제 / 가격·환리스크 / 유통·바이어 / 경쟁·브랜드 구도 등)에 배치합니다.',
        '현지 언론·여론 조사는 별도의 실시간 검색 담당(Perplexity)이 전담하므로, 6개 분석 좌석에는 현지언론·여론 렌즈를 배정하지 마세요. 경쟁 좌석은 경쟁사·브랜드 포지셔닝에 집중하도록 하세요.',
      ]
    : [
        '당신은 국가 자원·에너지 안보 개방형(라이트) 정책 브리핑의 오케스트레이터입니다.',
        '이 모드는 찬반 토론이 아닙니다. 정책결정자·공공기관이 "지금 가장 시급한 자원·에너지 현안은?" 같은 개방형 질문에 답하기 위해,',
        '6개 추론 AI를 각기 다른 중립적 분석 렌즈(에너지 수급·안보 / 유가·가격 / 국제정세·지정학 / 산업 영향 / 물가·거시 등)에 배치합니다.',
      ]
}
...
export function synthesisPersonaLines(m: JejuCouncilMode): string[] {
  return isTrade(m)
    ? [
        ...
        '6개 AI의 병렬 분석과 사전 브리핑을 읽고, 수출기업이 바로 실행 의사결정에 쓸 수 있는',
        '하나의 통합 브리핑을 작성하세요.',
      ]
    : [
        '당신은 개방형 자원·에너지 안보 정책 브리핑의 최종 통합 작성자(총괄 정책 보좌)입니다.',
        '6개 AI의 병렬 분석과 사전 브리핑을 읽고, 정책결정자·공공기관이 바로 정책 판단에 쓸 수 있는',
        '하나의 통합 브리핑을 작성하세요.',
      ]
}
