...
import { getExchangeRates } from '@/lib/motie/exchange'
...
// ─────────────────────────────────────────────────────────────────────────────
// TRADE mode sources (수출참모): KOTRA 국가정보 + KOTRA 상품DB + 환율(수출입은행).
//
// councilMode==='trade' gathers THESE instead of the governance sources (see
// lib/motie/brief.ts TRADE_SOURCE_IDS). All three are defensive — they never
// throw and any failure becomes an ok:false snapshot entry — so the engine still
// runs if one is unavailable. Keys come strictly from process.env (never
// hardcoded): KOTRA uses the shared data.go.kr portal key; 환율 uses its own
// KOREAEXIM_API_KEY via lib/motie/exchange (same as tourist mode).
// ─────────────────────────────────────────────────────────────────────────────
...
/** Curated top Korea export markets (ISO 3166-1 alpha-2) for KOTRA 국가정보. */
const TRADE_EXPORT_MARKETS: ReadonlyArray<{ iso2: string; ko: string }> = [
  { iso2: 'CN', ko: '중국' },
  { iso2: 'US', ko: '미국' },
  { iso2: 'VN', ko: '베트남' },
  { iso2: 'JP', ko: '일본' },
...
/**
 * Loosely extracts an item array from whatever envelope a data.go.kr / KOTRA
 * endpoint returns. KOTRA's exact response shape is not published in a stable
 * form, so we probe the common shapes (odcloud data[], flat items[]/item[],
 * nested response.body.items.item AND response.body.itemList.item [KOTRA]).
 * Returns [] on miss.
 */
function extractItemsLoose(parsed: unknown): Record<string, unknown>[] {
  ...
  if (Array.isArray(root.item)) return root.item as Record<string, unknown>[]
  ...
  if (body && typeof body === 'object') {
    ...
    // Standard data.go.kr: response.body.items.item[]
    if (Array.isArray(b.items)) return b.items as Record<string, unknown>[]
    const itemsC = b.items
    if (itemsC && typeof itemsC === 'object') {
      const it = (itemsC as Record<string, unknown>).item
      if (Array.isArray(it)) return it as Record<string, unknown>[]
      if (it && typeof it === 'object') return [it as Record<string, unknown>]
    }

    // KOTRA APIs: response.body.itemList.item[] (or single object)
    const itemList = b.itemList
    if (itemList && typeof itemList === 'object') {
      const it = (itemList as Record<string, unknown>).item
      if (Array.isArray(it)) return it as Record<string, unknown>[]
      if (it && typeof it === 'object') return [it as Record<string, unknown>]
    }

    if (Array.isArray(b.item)) return b.item as Record<string, unknown>[]
    if (b.item && typeof b.item === 'object') return [b.item as Record<string, unknown>]
  }

  // Single flat object with scalar fields → treat as one item.
  const hasScalar = Object.values(root).some(
    (v) => typeof v === 'string' || typeof v === 'number'
  )
  return hasScalar ? [root] : []
}

// ── KOTRA 국가정보 dedicated renderer ─────────────────────────────────────────
//
// Field map confirmed by live probe (2026-07-03, VN response 235K chars).
// Only the economically-meaningful fields are surfaced; the rest (hotels,
// restaurants, schools, maps…) are silently skipped.

/** Safe string from an item field, HTML-entity-free, trimmed. */
function kotraStr(item: Record<string, unknown>, key: string, maxLen = 400): string {
  const v = item[key]
  if (typeof v !== 'string') return ''
  // Strip common HTML entities that KOTRA embeds in long-text fields.
  return v
    .replace(/&(nbsp|middot|rsquo|lsquo|ldquo|rdquo|amp|lt|gt|ndash|mdash|#39|#160);/gi, ' ')
    .replace(/<[^>]+>/g, '')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, maxLen)
...
/** Latest value from a `{ critYear, inditValds }[]` series list, e.g. nmgdpList.nmgdp[]. */
function kotraLatestSeries(
  item: Record<string, unknown>,
  listKey: string,
  innerKey: string
): { year: string; val: string } | null {
  const list = item[listKey]
  if (!list || typeof list !== 'object') return null
  const arr = (list as Record<string, unknown>)[innerKey]
  if (!Array.isArray(arr) || arr.length === 0) return null
  // Sort descending by critYear and take the most recent.
  const sorted = [...arr].sort((a, b) => {
    const ya = Number((a as Record<string, unknown>).critYear ?? 0)
    const yb = Number((b as Record<string, unknown>).critYear ?? 0)
    return yb - ya
  })
  const top = sorted[0] as Record<string, unknown>
  const year = String(top.critYear ?? '').trim()
  const val = String(top.inditValds ?? '').trim()
  return year && val ? { year, val } : null
}

/** Renders a KOTRA 국가정보 single-item blob into a compact trade-focused summary. */
function renderKotraNationItem(item: Record<string, unknown>, iso2: string, ko: string): string {
  const lines: string[] = [`### ${ko} (${iso2})`]

  // ── Basic facts ────────────────────────────────────────────────────────────
  const pop = item.poplCnt ? String(item.poplCnt).split(' ')[0] : ''
  const area = item.area ? String(item.area).split(' ')[0] : ''
  const capital = kotraStr(item, 'cptlNm', 30)
  const lang = kotraStr(item, 'langNm', 40)
  const korOvkr = item.korOvkrCnt ? String(item.korOvkrCnt).split(' ')[0] : ''
  const updDt = kotraStr(item, 'updDt', 12) || kotraStr(item, 'regDt', 12)

  if (capital) lines.push(`수도: ${capital}`)
  if (lang) lines.push(`언어: ${lang}`)
  if (pop) lines.push(`인구: ${Number(pop).toLocaleString()}명`)
  if (area) lines.push(`면적: ${Number(area).toLocaleString()}㎢`)
  if (korOvkr) lines.push(`재외한인: ${Number(korOvkr).toLocaleString()}명`)
  if (updDt) lines.push(`자료갱신: ${updDt}`)

  // ── Key economic indicators (latest year from time-series) ─────────────────
  const gdp = kotraLatestSeries(item, 'nmgdpList', 'nmgdp')
  const gdppc = kotraLatestSeries(item, 'gdpcpList', 'gdpcp')
  const growth = kotraLatestSeries(item, 'ecnmyGrwrtList', 'ecnmyGrwrt')
  const infl = kotraLatestSeries(item, 'inflRateList', 'inflRate')
  const unemp = kotraLatestSeries(item, 'unempRateList', 'unempRate')
  const fdi = kotraLatestSeries(item, 'frgnrDrinvAmtList', 'frgnrDrinvAmt')

  lines.push('')
  lines.push('[주요 경제지표]')
  if (gdp) lines.push(`  명목GDP(${gdp.year}): ${gdp.val}억 달러`)
  if (gdppc) lines.push(`  1인당GDP(${gdppc.year}): ${gdppc.val} USD`)
  if (growth) lines.push(`  경제성장률(${growth.year}): ${growth.val}%`)
  if (infl) lines.push(`  물가상승률(${infl.year}): ${infl.val}%`)
  if (unemp) lines.push(`  실업률(${unemp.year}): ${unemp.val}%`)
  if (fdi) lines.push(`  해외직접투자(FDI, ${fdi.year}): ${fdi.val}백만 달러`)
  const wage1 = item.clggdOfjfpAvgWage
  const wage2 = item.hsgrdMfjfpAvgWage
  if (wage1 || wage2) {
    lines.push(`  평균임금: 사무직 ${wage1 ?? '?'}USD / 제조직 ${wage2 ?? '?'}USD (월)`)
  }
  const fxNote = kotraStr(item, 'critEhgt', 80) || kotraStr(item, 'prcsInfoUsdVal', 40)
  if (fxNote) lines.push(`  환율: ${fxNote}`)

  // ── Economic overview (short excerpts) ─────────────────────────────────────
  const ecnmyTrend = kotraStr(item, 'ecnmyTrendCntnt', 300)
  const ecnmyPrspt = kotraStr(item, 'ecnmyPrsptCntnt', 200)
  const mrkt = kotraStr(item, 'mrktChrtrtCntnt', 200)
  const ecnmy = kotraStr(item, 'ecnmyCntnt', 200)

  if (ecnmyTrend) { lines.push(''); lines.push(`[경제동향] ${ecnmyTrend}`) }
  if (ecnmyPrspt) { lines.push(`[경제전망] ${ecnmyPrspt}`) }
  if (mrkt) { lines.push(`[시장특성] ${mrkt}`) }
  if (ecnmy) { lines.push(`[한국교역] ${ecnmy}`) }

  // ── Korea → this country: top 10 export commodities ───────────────────────
  const xportList = item.korTp10cXportCmdltList
  if (xportList && typeof xportList === 'object') {
    const arr = (xportList as Record<string, unknown>).korTp10cXportCmdlt
    if (Array.isArray(arr) && arr.length > 0) {
      lines.push('')
      lines.push('[한국 수출 상위 품목 (대한수출입)]')
      const rows = arr as Record<string, unknown>[]
      for (const r of rows.slice(0, 5)) {
        const nm = String(r.cmdltNm ?? '').trim()
        const amt = String(r.xportAmt ?? '').trim()
        const yr = String(r.yearTitl ?? '').trim()
        if (nm) lines.push(`  - ${nm}: ${amt ? `${amt}백만 달러` : ''}${yr ? ` (${yr})` : ''}`)
      }
    }
  }

  // ── Korea's import regulations in this country ─────────────────────────────
  const reglList = item.korImprtReglList
  if (reglList && typeof reglList === 'object') {
    const arr = (reglList as Record<string, unknown>).korImprtRegl
    if (Array.isArray(arr) && arr.length > 0) {
      lines.push('')
      lines.push('[대한 수입규제 현황]')
      const rows = arr as Record<string, unknown>[]
      for (const r of rows.slice(0, 5)) {
        const nm = String(r.cmdltName ?? '').trim()
        const regl = String(r.reglCn ?? '').trim()
        const rate = kotraStr(r, 'lastRsltTxrtVal', 80)
        const period = kotraStr(r, 'lastRsltPdVal', 60)
        if (nm) lines.push(`  - ${nm}: ${regl}${rate ? ` / ${rate}` : ''}${period ? ` / ${period}` : ''}`)
      }
    }
  }

  // ── This country's top export/import partners ──────────────────────────────
  const xportPartners = item.xportTp10cList
  if (xportPartners && typeof xportPartners === 'object') {
    const arr = (xportPartners as Record<string, unknown>).xportTp10c
    if (Array.isArray(arr) && arr.length > 0) {
      const top3 = (arr as Record<string, unknown>[]).slice(0, 3)
        .map((r) => `${String(r.crsjNatnNm ?? '')}`)
        .filter(Boolean)
      if (top3.length) lines.push(`[주요 수출국(1~3위)]: ${top3.join(' → ')}`)
    }
  }
  const imprtPartners = item.imprtTp10cList
  if (imprtPartners && typeof imprtPartners === 'object') {
    const arr = (imprtPartners as Record<string, unknown>).imprtTp10c
    if (Array.isArray(arr) && arr.length > 0) {
      const top3 = (arr as Record<string, unknown>[]).slice(0, 3)
        .map((r) => `${String(r.crsjNatnNm ?? '')}`)
        .filter(Boolean)
      if (top3.length) lines.push(`[주요 수입국(1~3위)]: ${top3.join(' → ')}`)
    }
  }

  // ── FTAs / Treaties ────────────────────────────────────────────────────────
  const ftaList = item.cnclsTrtyList
  if (ftaList && typeof ftaList === 'object') {
    const arr = (ftaList as Record<string, unknown>).cnclsTrty
    if (Array.isArray(arr) && arr.length > 0) {
      const names = (arr as Record<string, unknown>[]).slice(0, 4)
        .map((r) => String(r.cnclsTrtyNm ?? '').trim())
        .filter(Boolean)
      if (names.length) lines.push(`[체결 FTA/협정]: ${names.join(', ')}`)
    }
  }

  // ── Korean investment ──────────────────────────────────────────────────────
  const korinvList = item.korCompInvtAmtList
  if (korinvList && typeof korinvList === 'object') {
    const arr = (korinvList as Record<string, unknown>).korCompInvtAmt
    if (Array.isArray(arr) && arr.length > 0) {
      const r = arr[0] as Record<string, unknown>
      const yr = String(r.critYear ?? '').trim()
      const amt = String(r.invtAmt ?? '').trim()
      const cnt = String(r.newCprCnt ?? '').trim()
      if (yr && amt) lines.push(`[한국기업 투자(${yr})]: ${amt}천달러, 신규법인 ${cnt}개`)
    }
  }
...
/**
 * KOTRA 국가정보 (대한무역투자진흥공사_국가정보) — per-country business/market
 * profile (GDP·인구·수출유망품목·대한수입규제 등). Fetched for a curated set of top
 * export markets in parallel and rendered into a compact Korean trade summary.
 * Endpoint (data.go.kr org B410001):
 *   .../kotra_nationalInformation/natnInfo/natnInfo?serviceKey&type=json&isoWd2CntCd=XX
 * Never throws.
 */
async function fetchKotraNationInfo(): Promise<{ text: string } | { error: string }> {
  const key = dataGoKrKey()
  if (!key) return { error: 'data.go.kr 서비스키가 설정되지 않았습니다 (DATA_GO_KR_KEY).' }

  const settled = await Promise.all(
    TRADE_EXPORT_MARKETS.map(async ({ iso2, ko }) => {
      const params = new URLSearchParams({ serviceKey: key, type: 'json', isoWd2CntCd: iso2 })
      const url = `https://apis.data.go.kr/B410001/kotra_nationalInformation/natnInfo/natnInfo?${params.toString()}`
      const r = await fetchJsonAt(url)
      if (!r.ok) return { iso2, ko, ok: false as const, error: r.error }
      const items = extractItemsLoose(r.parsed)
      return { iso2, ko, ok: true as const, items }
    })
  )
  ...
  for (const s of settled) {
    if (!s.ok) {
      failures.push(`- ${s.ko}(${s.iso2}): ${s.error}`)
      continue
    }
    if (s.items.length === 0) {
      failures.push(`- ${s.ko}(${s.iso2}): 데이터 없음`)
      continue
    }
    try {
      blocks.push(renderKotraNationItem(s.items[0]!, s.iso2, s.ko))
    } catch {
      failures.push(`- ${s.ko}(${s.iso2}): 렌더링 오류`)
    }
  }

  if (blocks.length === 0) {
    return { error: `KOTRA 국가정보: 수집 실패 — ${failures.join('; ') || '알 수 없는 오류'}` }
  }

  const header = `KOTRA 국가정보 — 주요 수출시장 프로필 (출처: data.go.kr B410001 국가정보, 대상국 ${TRADE_EXPORT_MARKETS.map((m) => m.iso2).join('·')})\n`
  const notes = failures.length ? [`\n[수집 실패/누락]\n${failures.join('\n')}`] : []
  return { text: [header, ...blocks, ...notes].join('\n\n') }
}

/**
 * KOTRA 상품DB (대한무역투자진흥공사_상품DB) — 국가별 현지 유망상품. Fetches the most
 * recent entries across all markets and renders using the human-readable summary
 * fields. Skips the large `newsBdt` HTML blob entirely.
 * Endpoint (data.go.kr org B410001):
 *   .../cmmdtDb/cmmdtDb?serviceKey&type=json&numOfRows&pageNo[&search1=country]
 * Never throws.
 */
async function fetchKotraProductDb(): Promise<{ text: string } | { error: string }> {
  const key = dataGoKrKey()
  if (!key) return { error: 'data.go.kr 서비스키가 설정되지 않았습니다 (DATA_GO_KR_KEY).' }

  // Fetch 2 pages in parallel: one unfiltered (latest global), one per-country for
  // each of our target markets. Merge and deduplicate by title.
  const ROWS_PER_CALL = 10
  const marketQueries = TRADE_EXPORT_MARKETS.slice(0, 4).map(({ ko }) => ko)

  const fetches = [
    // Page 1 — no country filter (global latest)
    (async () => {
      const params = new URLSearchParams({ serviceKey: key, type: 'json', numOfRows: String(ROWS_PER_CALL), pageNo: '1' })
      const r = await fetchJsonAt(`https://apis.data.go.kr/B410001/cmmdtDb/cmmdtDb?${params.toString()}`)
      return r.ok ? extractItemsLoose(r.parsed) : []
    })(),
    // Per-market country queries
    ...marketQueries.map((countryKo) =>
      (async () => {
        const params = new URLSearchParams({ serviceKey: key, type: 'json', numOfRows: String(ROWS_PER_CALL), pageNo: '1', search1: countryKo })
        const r = await fetchJsonAt(`https://apis.data.go.kr/B410001/cmmdtDb/cmmdtDb?${params.toString()}`)
        return r.ok ? extractItemsLoose(r.parsed) : []
      })()
    ),
  ...
  // Merge + deduplicate by newsTitl (title).
  const seen = new Set<string>()
  const all: Record<string, unknown>[] = []
  for (const items of settled) {
    for (const it of items) {
      const title = typeof it.newsTitl === 'string' ? it.newsTitl.trim() : ''
  ...
  if (all.length === 0) return { error: 'KOTRA 상품DB: 데이터 없음 (빈 응답)' }

  const RENDER_CAP = 25
  const lines = all.slice(0, RENDER_CAP).map((it, i) => {
    const title = typeof it.newsTitl === 'string' ? it.newsTitl.trim() : ''
    const regn = typeof it.regn === 'string' ? it.regn.trim() : ''
    const cmdlt = typeof it.cmdltNmKorn === 'string' ? it.cmdltNmKorn.trim().slice(0, 50) : ''
    const hsCd = typeof it.hsCdNm === 'string' ? it.hsCdNm.trim().slice(0, 30) : ''
    const summary = typeof it.cntntSumar === 'string'
      ? it.cntntSumar.replace(/&[a-z#0-9]+;/gi, ' ').replace(/\s+/g, ' ').trim().slice(0, 250)
      : ''
    const meta = [regn, cmdlt, hsCd].filter(Boolean).join(' · ')
    return [
      `- (${i + 1}) [${title}]${meta ? ` (${meta})` : ''}`,
      summary ? `  요약: ${summary}` : '',
    ].filter(Boolean).join('\n')
  ...
  const header = `KOTRA 상품DB — 국가별 현지 유망상품 (출처: data.go.kr B410001 상품DB, ${Math.min(all.length, RENDER_CAP)}건 표시)`
  return { text: [header, ...lines].join('\n') }
}

/**
 * 환율 (한국수출입은행) — KRW reference rates for trade counterparties. Reuses the
 * tourist-mode fetch logic via lib/motie/exchange (oapi.koreaexim.go.kr, 2025.6
 * domain). Never throws.
 */
async function fetchKoreaeximFx(): Promise<{ text: string } | { error: string }> {
  const r = await getExchangeRates()
  if (!r.ok) return { error: `환율 수집 실패: ${r.error}` }
  if (r.data.rates.length === 0) return { error: '환율 데이터 없음' }

  const lines = r.data.rates.map(
    (rate) => `- ${rate.code}: ${rate.rate.toLocaleString()}원${rate.unit > 1 ? ` / ${rate.unit}단위` : ''}`
  )
  const header = `원화 환율 기준 (한국수출입은행, 기준일 ${r.data.date})`
  return { text: [header, ...lines].join('\n') }
}
...
const JEJU_SOURCES: readonly JejuSource[] = [
  ...
  // ── TRADE mode (수출참모) sources ─────────────────────────────────────────
  {
    id: 'kotra-nation-info',
    label: 'KOTRA 국가정보 (주요 수출시장 프로필)',
    format: 'json',
    modes: ['trade'],
    // Nominal primary endpoint (listing/debugging). The real work — a per-country
    // loop over TRADE_EXPORT_MARKETS — happens in fetchCustom.
    buildUrl: () => {
      const params = new URLSearchParams({ serviceKey: dataGoKrKey(), type: 'json', isoWd2CntCd: 'US' })
      return `https://apis.data.go.kr/B410001/kotra_nationalInformation/natnInfo/natnInfo?${params.toString()}`
    },
    fetchCustom: fetchKotraNationInfo,
  },

  {
    id: 'kotra-product-db',
    label: 'KOTRA 상품DB (국가별 유망상품)',
    format: 'json',
    modes: ['trade'],
    buildUrl: () => {
      const params = new URLSearchParams({ serviceKey: dataGoKrKey(), type: 'json', numOfRows: '20', pageNo: '1' })
      return `https://apis.data.go.kr/B410001/cmmdtDb/cmmdtDb?${params.toString()}`
    },
    fetchCustom: fetchKotraProductDb,
  },

  {
    ...
    format: 'json',
    modes: ['trade'],
    // Nominal URL only — the actual fetch runs through lib/motie/exchange
    // (oapi.koreaexim.go.kr) inside fetchCustom, using KOREAEXIM_API_KEY.
    buildUrl: () => 'https://oapi.koreaexim.go.kr/site/program/financial/exchangeJSON',
    fetchCustom: fetchKoreaeximFx,
  },

  // ── WARROOM mode (자원·에너지 워룸) sources ─────────────────────────────────
  {
    id: 'opinet-fuel-prices',
    label: '오피넷 전국 평균 유가 (한국석유공사)',
    format: 'json',
    modes: ['warroom'],
    // Nominal primary endpoint (listing/debugging). fetchCustom calls both
    // avgAllPrice.do + avgRecentPrice.do with OPINET_API_KEY.
    buildUrl: () =>
      `http://www.opinet.co.kr/api/avgAllPrice.do?out=json&code=${encodeURIComponent(process.env.OPINET_API_KEY ?? '')}`,
    fetchCustom: fetchOpinetFuelPrices,
  },
...

  {
    ...
    modes: ['warroom'],
    buildUrl: () => {
      const params = new URLSearchParams({ page: '1', perPage: '600', serviceKey: dataGoKrKey() })
      return `https://api.odcloud.kr/api/15088508/v1/${KOGAS_LNG_UDDI}?${params.toString()}`
    },
    fetchCustom: fetchKogasLng,
  },
