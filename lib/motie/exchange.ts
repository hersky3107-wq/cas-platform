...
/**
 * Live exchange rates — Korea Eximbank open API.
 *
 * ISOLATED MOTIE COPY of lib/jeju/exchange.ts (same fetch logic/endpoint/quirks),
 * kept inside the motie tree so the AX COUNCIL data layer never imports lib/jeju.
 * Reuses the SAME env var the tourist connectors read (KOREAEXIM_API_KEY).
 *
...
/**
 * Currencies most relevant to Korea's export/trade counterparties, in display
 * order. `match` lists the raw cur_unit tokens that map to this display `code`
...
async function fetchForDate(compactDate: string): Promise<FetchOutcome> {
  ...
  if (!key) {
    console.error('[motie-exchange] KOREAEXIM_API_KEY not configured')
    return { status: 'error' }
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[motie-exchange] HTTP ${res.status} for ${masked(url)}`)
      return { status: 'error' }
    }
    ...
    try {
      ...
      console.error(`[motie-exchange] unparseable response for ${masked(url)}: ${text.slice(0, 160)}`)
      return { status: 'error' }
    }
    ...
    if (code === 3) {
      console.error('[motie-exchange] auth error (result=3) — check KOREAEXIM_API_KEY')
      return { status: 'auth' }
    }
    if (code === 4) {
      console.error('[motie-exchange] daily request limit reached (result=4)')
      return { status: 'auth' }
    }
    ...
    console.error(`[motie-exchange] ${aborted ? 'timeout' : 'fetch error'} for ${masked(url)}`)
    return { status: 'error' }
  } finally {
    clearTimeout(timer)
  }
...
/**
 * Live KRW reference rates for the curated currencies, using the most recent
 * business day with data. Cached in-memory for the current KST day.
 */
