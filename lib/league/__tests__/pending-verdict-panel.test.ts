...
describe('PendingVerdictPanel — pre-grading prediction axes', () => {
  ...
  it('labels the axes as predictions, not hits, and never uses a slash-over-total', () => {
    ...
    expect(html).toContain('9 up')
    expect(html).toContain('5 down')
    expect(html).toContain('Own reasoning')
    expect(html).toContain('10 up')
    expect(html).toContain('Closed-weights')
    expect(html).toContain('8 up')
    expect(html).not.toMatch(/[✓✗]/)
    expect(html).not.toMatch(/\d+\/\d+/)
  })
})
