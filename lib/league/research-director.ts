...
export function buildPacketInventory(input: PacketInventoryInput): string {
  ...
  if (!slow) {
    ...
    for (const [key, field] of [
      ...
      ['eia_us_crude_stocks_supply', slow.eiaCrude],
      ['eia_us_natgas_storage', slow.eiaNatgas],
      ['cot_wti', slow.cotWti],
      ['cot_brent', slow.cotBrent],
      ['cot_natgas', slow.cotNatgas],
      ['ovx_crude_vol', slow.ovx],
      ['fred_wti_spot', slow.wtiSpotFred],
      ['fred_brent_spot', slow.brentSpotFred],
      ['fred_henry_hub_spot', slow.henryHubSpotFred],
      ['fred_retail_gasoline', slow.gasolineRetail],
      ['cot_copper', slow.cotCopper],
      ['cot_corn', slow.cotCorn],
      ['cot_wheat', slow.cotWheat],
      ['cot_soybean', slow.cotSoybean],
      ['cot_coffee', slow.cotCoffee],
      ['fred_imf_copper', slow.copperSpotFred],
      ['fred_imf_corn', slow.cornSpotFred],
      ['fred_imf_wheat', slow.wheatSpotFred],
      ['fred_imf_soybean', slow.soybeanSpotFred],
      ['fred_imf_coffee', slow.coffeeSpotFred],
      ['fed_funds', slow.fedFunds],
      ['ust_2y', slow.ust2y],
      ['ust_10y', slow.ust10y],
      ['ust_10y2y', slow.ust10y2y],
      ['fred_tips_10y', slow.tips10yFred],
      ['dxy_broad', slow.dxyBroad],
      ['dxy_afe', slow.dxyAfe],
      ['dxy_eme', slow.dxyEme],
      ['ecb_deposit', slow.ecbDeposit],
      ['ecb_refi', slow.ecbRefi],
      ['german_bund_10y', slow.germanBund10y],
      ['euro_hicp', slow.euroHicp],
      ['boj_policy', slow.bojPolicy],
      ['jgb_10y', slow.jgb10y],
      ['tibor_3m', slow.tibor3m],
      ['bok_rate', slow.bokRate],
      ['ktb_10y', slow.ktb10y],
      ['krw_cd_3m', slow.krwCd3m],
      ['krw_cpi', slow.krwCpi],
      ['sonia', slow.sonia],
      ['gilt_10y', slow.gilt10y],
      ['fx_policy_rate_diff', slow.policyRateDiff],
      ['fx_10y_yield_diff', slow.yield10yDiff],
      ['cot_eur', slow.cotEur],
      ['cot_jpy', slow.cotJpy],
      ['cot_gbp', slow.cotGbp],
      ['cot_aud', slow.cotAud],
