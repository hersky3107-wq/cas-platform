...
import type { JejuSnapshot } from '@/lib/jeju/brief'
import {
  mergeSearchRequests,
  executeJejuSearches,
...
function buildLeadAnalysisSystemPrompt(): string {
  return [
    ...
    '- 내부 데이터만으로 부족한 부분, 즉 최신 외부 정보(타지역 사례, 정부 정책, 법령, 시장·수출 동향, 반도체 실적 등)가 필요한 지점을 명확히 짚고, 추측으로 메우지 말고 "검색 요청"으로 선언하세요.',
...
/** Korean section list for the final report, branched by mode. */
function reportSectionList(mode: JejuPreReportMode): string {
  const shared = [
    '1. 핵심 현안: 가장 중요한 메시지 3~5줄로 요약.',
    '2. 데이터가 말하는 것: 확보된 정량·정성 데이터의 해석. 각 수치·사실마다 반드시 출처(KPX, KAMIS, KMA, 감귤통계, 항만 물동량, 관광객 통계 등)와 데이터 시점(언제 기준인지)을 괄호로 함께 명시하세요. 예: "태양광 발전 비중 OO% (출처: KPX, 2026-06-27 기준)". 출처·시점을 밝힐 수 없는 수치는 쓰지 마세요. 각 데이터 포인트는 1~2줄로 간결하게, 중복 없이.',
...
export async function generateJejuPreReport(params: {
  ...
  try {
    const r = await runSingleAiProvider({
      ...
      userId: null,
      provider: LEAD_PROVIDER,
      prompt: ['[거버넌스 질문]', question, '', '[수집 데이터]', context, '', '위 데이터를 분석하고 필요한 검색을 선언하세요. 스키마에 맞는 순수 JSON만 출력하세요.'].join('\n'),
      systemPrompt: buildLeadAnalysisSystemPrompt(),
  ...
  const writerPrompt = [
    ...
    '',
    '위 자료를 근거로, 지정된 섹션 구조의 한국어 거버넌스 분석 리포트를 작성하세요. 모든 수치에 출처와 시점을 병기하세요.',
  ]
    .filter((line) => line !== '')
    .join('\n')
  ...
  try {
    const r = await runSingleAiProvider({
      ...
      provider: LEAD_PROVIDER,
      prompt: writerPrompt,
      systemPrompt: buildReportWriterSystemPrompt(mode, okSourceLabels(params.snapshot)),
