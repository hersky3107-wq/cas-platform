...
function buildSearchSystemPrompt(today: string): string {
  return [
    ...
    '',
    '기간 제한(중요, 최근 7일 이내): 오늘 기준 최근 7일 이내에 보도된 기사만 다루세요. 7일보다 오래된 기사는 결과에 포함하지 마세요. 가장 최신 보도부터 우선 찾고, 오늘 → 어제 → 그 이전 날짜 순으로 훑어보세요.',
    '',
    ...
    '③ 보도 날짜: 반드시 구체적으로(가능하면 YYYY-MM-DD) 명시하세요 — 다음 종합 분석 단계가 이 날짜를 기준으로 최신순 정렬을 하므로 날짜 표기가 필수입니다. 날짜를 확정할 수 없으면 "날짜 불명", 7일이 넘은 오래된 기사이면 "오래된 기사"로 표시하고 절대 지어내지 마세요.',
    '④ 논조(framing): 이 보도가 사안을 어떻게 바라보는지(긍정적/부정적/중립적, 무엇을 강조하고 무엇을 우려하는지) 1~2문장으로 설명하세요.',
    '가능하면 출처 URL도 포함하세요.',
    '',
    '가능하면 제주 지역언론과 전국 언론을 섞어서 폭넓게 보세요.',
    '규칙(엄수): 실재하지 않는 기사·날짜·수치를 절대 지어내지 마세요. 지지율·찬성률 같은 정량 수치는 만들지 말고, 보도 논조는 정성적으로만 다루세요. 해당 영역에서 최근 7일 이내 보도를 찾지 못했으면, 7일이 넘은 오래된 기사로 대체하지 말고 "이 영역에서 최근 7일 이내 제주 관련 보도를 찾지 못했습니다"라고 솔직히 쓰세요.',
  ].join('\n')
}
...
function buildSynthesisSystemPrompt(mode: JejuMediaWatchMode, today: string): string {
  return [
    ...
    '',
    `정렬 규칙(매우 중요 — 최신순): coreIssues와 브리핑 전체는 보도 날짜 기준 "최신순"으로 배열하세요. 가장 최근 보도(이상적으로는 오늘 ${today} 기준)가 맨 앞에 와야 합니다.`,
    '우선순위 단계(fallback): 오늘자 보도가 있으면 오늘 보도를 맨 앞에 놓으세요. 오늘자 보도가 없으면 어제 보도를 앞세우고, 어제도 없으면 그 전날 순으로, 최근 7일 윈도우 안에서 하루씩 거슬러 올라가며 "가장 신선한 자료"를 항상 맨 위에 배치하세요.',
    '최신성이 보도량보다 우선합니다: 여러 매체가 다뤘지만 날짜가 오래된 사안보다, 단 하나뿐이라도 더 신선한(오늘/어제) 보도를 앞에 배치하세요. 오래된 사안도 여전히 다룰 수 있지만 더 아래에 배치하고 날짜를 명시하세요. 검색 자료에 7일 윈도우를 벗어난 기사가 섞여 있다면 "[과거 자료]"로 표시하고, 날짜를 확정할 수 없는 자료는 "[시점 불명]"으로 표시하세요 — 이런 자료를 오늘의 화두처럼 맨 앞에 앞세우지 마세요.',
    '',
    perspectiveFor(mode),
    '',
    ...
    '스키마:',
    '{',
    '  "summary": "최상단 요약 (4~6줄). 오늘 제주 언론 지형의 큰 그림 — 가장 뜨거운 사안 2~3개와 전체 논조 분위기. 가장 최신 사안을 먼저 언급하세요.",',
    '  "coreIssues": "핵심 이슈 — 여러 매체에서 크게 다뤄진 주요 사안들. 반드시 위 정렬 규칙대로 보도 날짜 기준 최신순(오늘→어제→그 이전)으로 나열하세요. 각 사안마다: (1) 어떤 매체들이 어떤 날짜에 보도했는지, (2) 각 보도의 논조(긍정/부정/중립, 강조점, 우려), (3) 매체 간 논조 차이가 있으면 대비, (4) 이 사안이 정책적으로 왜 중요한지. 사안이 여럿이면 각각 충분히 다루세요. 이 섹션에 가장 큰 비중을 두세요.",',
    '  "minorIssues": "주변 이슈 — 보도량은 적지만 놓치면 안 될 사안들. 여기도 최신순으로 나열하세요. 각 항목마다 매체·날짜·논조를 한 줄~두 줄로. 항목이 여럿이면 모두 포함하세요.",',
    ...
    '}',
    '',
    '정직성 규칙(최우선): 제공된 보도 자료에만 근거하세요. 실재하지 않는 기사·수치·반응을 절대 지어내지 마세요. 지지율·찬성률 등 정량 수치를 만들지 말고 논조는 정성적으로만 다루세요. 특정 영역 보도가 실제로 빈약했다면 억지로 채우지 말고 "이 영역은 수집된 보도가 적었습니다"라고 솔직히 한 줄로 쓰고 넘어가세요. 분량은 반드시 실제 자료에서 나와야 합니다. 정렬을 맞추기 위해 날짜를 지어내거나 바꾸지 마세요 — 정렬은 오직 검색 자료에 실제로 표기된 날짜에만 근거하세요.',
  ].join('\n')
}
