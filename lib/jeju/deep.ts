...
function buildOrchestratorSystemPrompt(): string {
  ...
  return [
    ...
    '(e) 언론 분석가(press analyst) 역할: 이 사안이 도민·시민에게 영향을 주는 공공 정책 사안이면(특히 questionType이 "binary"이거나 사회적으로 쟁점화될 수 있는 주제이면), 3~5개 역할 중 하나로 "언론 분석가"를 포함하는 것을 적극 고려하세요.',
    '    - 모든 질문에 필수는 아닙니다. 좁은 기술·데이터 질문이라면 생략해도 됩니다. 그러나 공공성이 큰 정책 사안에는 강력히 권장됩니다.',
