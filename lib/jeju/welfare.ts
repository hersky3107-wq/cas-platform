/**
 * Welfare matching engine (region-agnostic).
 *
 * Reads region-scoped welfare records from Supabase and ranks them against a
 * resident's intake questionnaire using weighted scoring (NOT strict AND
 * filtering) — so an answered profile never yields an empty list.
 *
 * The default region is 'jeju', but the same logic serves a future national
 * dataset by passing a different `region`.
 */
...
/** A row as stored in the jeju_welfare_services table. */
...
export interface WelfareMatch {
  ...
  score: number
  /** Source identifier — e.g. 'jeju-d01' or 'national'. Useful for UI labelling. */
  source: string
