...
export async function getSubscriptionForUser(
  userId: string
): Promise<Pick<SubscriptionRow, 'plan_type' | 'status' | 'paypal_subscription_id'> | null> {
  const { data, error } = await supabaseAdmin
    .from('subscriptions')
    .select('plan_type, status, paypal_subscription_id')
    .eq('user_id', userId)
    .maybeSingle()
  ...
  if (!data) return null
  return data as Pick<SubscriptionRow, 'plan_type' | 'status' | 'paypal_subscription_id'>
}
