import {
  ORACLE_COMPAT_SINGLE_READER_COUNTS,
...
/** 단일 궁합: same seat resolution, compat reader counts (3/5) and prices. */
export function buildCompatSystemRosters(systemId: SystemId): ReadingRosterOption[] {
  return ORACLE_COMPAT_SINGLE_READER_COUNTS.map((readerCount) => {
    const roster = resolveSingleSystemRoster(systemId, readerCount);
    return {
      readerCount,
      readers: [...roster.readers],
      synthesizer: roster.synthesizer,
      credits: creditsForOracleSession("single", readerCount, "compat"),
    };
  });
}
