...
describe('daily prompts', () => {
  it('puts the 300–450 budget in the JSON schema and never advertises the parser slack', () => {
    ...
    expect(prompt).toContain('ninestar.흉방')
    ...
    expect(prompt).toContain('용신을 고정하지 않습니다')
    expect(prompt).toContain('삼구')
    expect(prompt).toContain('나왈')
    expect(prompt).toContain('메이저')
