...
describe('eokbu — talismanLean is additive', () => {
  it('does not change strength, yongsin, or inapplicable on 신약 / 신강 / 중화 / 종격', () => {
    const weak = eokbu(fourPillars({ date: '1988-03-15', time: '04:30', timezone: 'Asia/Seoul' }))
    const strong = eokbu(fourPillars({ date: '1984-02-10', time: '12:00', timezone: 'Asia/Seoul' }))
    const balanced = eokbu(fourPillars({ date: '1984-02-15', time: '12:00', timezone: 'Asia/Seoul' }))
    const jong = eokbu(fourPillars({ date: '1980-01-08', time: '04:30', timezone: 'Asia/Seoul' }))
    expect(weak.strength).toBe('weak')
    expect(weak.yongsin).toBe('fire')
    expect(weak.inapplicable).toBeNull()
    expect(strong.strength).toBe('strong')
    expect(strong.yongsin).toBe('fire')
    expect(balanced.strength).toBe('balanced')
    expect(balanced.yongsin).toBeNull()
    expect(jong.strength).toBeNull()
    expect(jong.yongsin).toBeNull()
    expect(jong.inapplicable?.code).toBe('jonggyeok_dominant')
    expect(weak.talismanLean).toEqual({ mode: 'fill', element: 'fire', intensity: 'full' })
    expect(strong.talismanLean).toEqual({ mode: 'drain', element: 'fire', intensity: 'full' })
    expect(balanced.talismanLean).toEqual({ mode: 'fill', element: 'fire', intensity: 'soft' })
    expect(jong.talismanLean).toEqual({ mode: 'follow', element: 'earth', intensity: 'full' })
  })

  it('breaking 중화 by 득령: 득령>0 is strong-lean drain', () => {
    const r = eokbu(fourPillars({ date: '1960-01-13', time: '12:00', timezone: 'Asia/Seoul' }))
    expect(r.strength).toBe('balanced')
    expect(r.yongsin).toBeNull()
    expect(r.deukryeong.score).toBeGreaterThan(0)
    expect(r.talismanLean).toEqual({ mode: 'drain', element: 'water', intensity: 'soft' })
  })
})
