/**
 * Welfare matching engine (region-agnostic) — national senior-care copy.
 *
 * COPIED from lib/jeju/welfare.ts so the Jeju original stays untouched. Reads the
 * shared jeju_welfare_services table with region IN (userRegion, 'national'):
 *   - userRegion — the residence-derived slug (residence.welfareRegionKey), e.g.
 *     'seoul' / 'gyeonggi' / 'jeju'. Local-government welfare tagged by
 *     scripts/tag-welfare-national.ts (source='local-national') is stored per slug.
