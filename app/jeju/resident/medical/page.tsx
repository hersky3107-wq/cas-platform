 ...
 *   - 약 알아보기        → /jeju/resident/photo?mode=medicine (built)
...
export default function MedicalPage() {
  ...
  const [onArea, setOnArea] = useState<'제주시' | '서귀포시' | null>(null)
  ...
  const submitSymptom = useCallback(async () => {
    ...
    setPhase('loading')
    try {
      const res = await fetch('/api/jeju/resident/symptom', {
        method: 'POST',
  ...
  }, [symptomText, listening, stopSpeaking])
  ...
  const submitOpenNow = useCallback(async (kind: '병원' | '약국', area: '제주시' | '서귀포시' | null) => {
    stopSpeaking()
    setOnError(null)
    setOnResult(null)
    setOnPhase('loading')
    try {
      const res = await fetch('/api/jeju/resident/open-now', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ kind, ...(area ? { area } : {}) }),
      })
  ...
  const resultSpeech = useCallback((r: SymptomResult): string => {
    ...
    if (list.length > 0) {
      parts.push(`제주에서 가실 수 있는 병원을 알려드릴게요.`)
                  ...
                  <>
                    ...
                    {(result.hospitals ?? []).length > 0 && (
                      <div style={styles.hospitalList}>
                        <h2 style={styles.listHeading}>제주에서 가실 수 있는 병원</h2>
          ...
          <>
            ...
            {onPhase === 'select' && (
              <section style={styles.card}>
                ...
                <p style={styles.areaLabel}>어느 지역인가요? <span style={{ fontWeight: 500, color: C.muted }}>(선택 — 기본: 제주도 전체)</span></p>
                <div style={styles.areaRow}>
                  {(['제주도 전체', '제주시', '서귀포시'] as const).map((a) => {
                    const val = a === '제주도 전체' ? null : a
                    const on = onArea === val
                    return (
                      <button
                        key={a}
                        type="button"
                        className="md-area"
                        style={on ? { ...styles.areaBtn, ...styles.areaBtnOn } : styles.areaBtn}
                        onClick={() => setOnArea(val)}
                        aria-pressed={on}
                      >
                        {a}
                      </button>
                ...
                <button
                  ...
                  onClick={() => onKind && submitOpenNow(onKind, onArea)}
                  disabled={!onKind}
