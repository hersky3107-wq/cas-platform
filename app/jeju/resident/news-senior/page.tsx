...
/**
 * 오늘의 소식 (어르신) — restored elderly-friendly news page for Jeju senior mode.
 *
 * Dedicated to /jeju/resident/senior — NOT shared with the general 도민 mode
 * (which has its own adult-density page at /jeju/resident/news, backed by
 * /api/domin/news). This page fetches /api/jeju/resident/news — the original
 * elderly-friendly API (jeju_resident_news cache, Perplexity-grounded,
 * Claude-rewritten into plain Korean) — restoring content that predates the
 * general-mode news page reusing this route.
 *
 * Presents news for LISTENING first: a big "전체 읽어주기" reads the whole
 * broadcast in order (national by section, then Jeju), plus per-item 🔊
 * buttons.
 *
...
const styles: Record<string, React.CSSProperties> = {
  ...
  fresh: {
    ...
    background: '#DCE9F7', borderRadius: 10, padding: '6px 16px',
  },
  // read-all
  readAllBtn: {
    ...
    boxShadow: '0 6px 20px rgba(14,78,138,0.28)',
  },
  // sections
  ...
  newsCard: {
    display: 'block', background: C.surface, border: `2px solid #D9C6A2`, borderRadius: 16,
  ...
  itemReadBtn: {
    flexShrink: 0, width: 56, height: 56, fontSize: 24, color: C.sea, background: '#EAF2FB',
  ...
  sourceList: {
    ...
    background: '#F5EAD6', borderRadius: 12, padding: '14px 16px', marginTop: 6,
  },
...
const GLOBAL_CSS = `
  ...
  .nw-item-read:hover { background: #DCE9F7; }
