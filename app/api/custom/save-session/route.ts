...
import { parseCustomResponses, type CustomSessionResponse } from '@/lib/custom/session-types'
...
function parseResponsesBody(raw: unknown): CustomSessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parseCustomResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    if (error || !data) {
      console.error('[custom/save-session] insert failed:', error?.message)
    ...
    console.error('[custom/save-session] POST', msg)
...
export async function PATCH(req: Request) {
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error'
    console.error('[custom/save-session] PATCH', msg)
    return NextResponse.json({ error: msg }, { status: 500 })
  }
}
