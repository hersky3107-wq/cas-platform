...
/**
 * 오늘의 소식 — elderly-friendly daily news for the national senior-care copy.
 *
 * COPIED from app/api/jeju/resident/news. Still uses the `jeju_resident_news`
...
출력 JSON 스키마(정확히 이 형태 — "jeju" 키는 위 지역 소식을 담는 칸입니다):
{
...
async function rewriteToPlain(nationalText: string, localText: string, regionLabel: string, apiKey: string): Promise<{ national: NationalItem[]; jeju: JejuItem[] } | null> {
...
export async function GET(req: Request) {
  ...
  // Region-specific cache key so each region caches separately (and never
  // collides with the Jeju app's region-less slots).
