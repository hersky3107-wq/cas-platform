...
export async function POST(req: Request) {
  ...
  // Residence-driven region (welfareRegionKey slug). Jeju keeps its regional
  // rows; other regions currently match only 'national' rows until regional
  // welfare DATA is added (next step). sido/sigungu are carried for that step.
  const region = typeof bodyObj.region === 'string' && bodyObj.region ? bodyObj.region : 'national'
  const sido = typeof bodyObj.sido === 'string' ? bodyObj.sido : null
  const sigungu = typeof bodyObj.sigungu === 'string' ? bodyObj.sigungu : null
  const limit =
  ...
  try {
    ...
    return NextResponse.json({ region, sido, sigungu, count: results.length, results })
  } catch (e) {
