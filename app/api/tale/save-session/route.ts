...
import { parseTaleResponses, type TaleSessionResponse } from '@/lib/tale/session-types'
...
function parseResponsesBody(raw: unknown): TaleSessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parseTaleResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    if (error || !data) {
      console.error('[tale/save-session] insert failed:', error?.message)
      return NextResponse.json(
    ...
    console.error('[tale/save-session] POST', msg)
...
export async function PATCH(req: Request) {
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error'
    console.error('[tale/save-session] PATCH', msg)
    return NextResponse.json({ error: msg }, { status: 500 })
  }
}
