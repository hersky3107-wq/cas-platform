...
import { parseOracleResponses, type OracleSessionResponse } from '@/lib/oracle/session-types'
...
function parseResponsesBody(raw: unknown): OracleSessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parseOracleResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    const oracle_type = typeof body.oracle_type === 'string' ? body.oracle_type.trim() : ''
    if (!oracle_type) {
      return NextResponse.json({ error: 'oracle_type is required' }, { status: 400 })
    }
    ...
    if (error || !data) {
      console.error('[oracle/save-session] insert failed:', error?.message)
      return NextResponse.json(
    ...
    console.error('[oracle/save-session] POST', msg)
...
export async function PATCH(req: Request) {
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error'
    console.error('[oracle/save-session] PATCH', msg)
    return NextResponse.json({ error: msg }, { status: 500 })
  }
}
