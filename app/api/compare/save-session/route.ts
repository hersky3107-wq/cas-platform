...
import { parseCompareResponses, type CompareSessionResponse } from '@/lib/compare/session-types'
...
function parseResponsesBody(raw: unknown): CompareSessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parseCompareResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    if (error || !data) {
      console.error('[compare/save-session] insert failed:', error?.message)
    ...
    console.error('[compare/save-session] POST', msg)
...
export async function PATCH(req: Request) {
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error'
    console.error('[compare/save-session] PATCH', msg)
    return NextResponse.json({ error: msg }, { status: 500 })
  }
}
