...
export async function POST(req: Request) {
  ...
  await insertVote(supabase, sessionId, {
    userId: user.id,
    choice: userVoteRaw,
  ...
  const vl = await supabase.from('debate_logs').insert([{ session_id: sessionId, role: 'user', message_text: voteLog }])
  if (vl.error) {
    await supabase.from('debate_logs').insert([{ session_id: sessionId, content: voteLog, speaker: 'user' }])
  }
