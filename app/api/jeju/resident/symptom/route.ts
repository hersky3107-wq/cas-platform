...
import { searchHospitals, type MedicalFacility } from '@/lib/jeju/hira'
...
const TRIAGE_SYSTEM = `당신은 제주 어르신을 돕는 의료 안내 도우미입니다. 진단하지 말고, 아래만 판단해 순수 JSON으로만 답하세요. 마크다운 없이.
...
const EXTRACT_SYSTEM = `아래 검색 결과 텍스트에서 제주 지역의 병·의원 이름과 위치, 종별을 추출해 순수 JSON 배열로만 출력하세요. 마크다운 없이.
...
export async function POST(req: Request) {
  ...
  const region = typeof body.region === 'string' ? body.region : 'jeju'
  ...
  const { text: pplxText, citations } = await askPerplexity(
    `제주도에서 "${department}" 진료를 받을 수 있는 동네 의원(1차 의료기관) 위주로 알려줘. ${tierHint} 각 병원의 이름과 위치(시·동)를 구체적으로 알려줘.`,
    {
      systemPrompt:
        '당신은 제주 지역 의료기관 정보를 찾는 검색 도우미입니다. 실제로 존재하는 기관만, 이름과 위치 위주로 구체적으로 한국어로 정리하세요. 동네 의원(의원급)을 우선하고, 추측하지 말고 확인되는 정보만 제시하세요.',
      maxTokens: 700,
    }
  )
  ...
  try {
    hira = await searchHospitals({ region, fetchLimit: 1500, limit: 1500 })
  } catch (e) {
