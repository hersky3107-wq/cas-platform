import { getJejuIslandsInfo } from '@/lib/jeju/tourist-ferry'
...
// ─────────────────────────────────────────────────────────────────────────────
// JEJU tourist ferry/islands route — mirrors app/api/jeju/tourist-festivals/route.ts.
// POST (empty body OK) → getJejuIslandsInfo({ today }) → JSON.
...
export async function POST(): Promise<Response> {
  ...
  try {
    const result = await getJejuIslandsInfo({ today })
