...
export async function POST(req: Request): Promise<Response> {
  ...
  // (`POST /api/league/deep-open` accepts only { roundId, locale? }). A
    ...
    return json(
      {
        ...
        error: 'League deep analysis must use POST /api/league/deep-open with { roundId } only.',
        code: 'use_league_deep_open',
      },
      400
    )
  }
