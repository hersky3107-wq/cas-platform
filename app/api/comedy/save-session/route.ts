...
import { parseComedyResponses, type ComedySessionResponse } from '@/lib/comedy/session-types'
...
function parseResponsesBody(raw: unknown): ComedySessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parseComedyResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    const comedy_type = typeof body.comedy_type === 'string' ? body.comedy_type.trim() : ''
    if (!comedy_type) {
      return NextResponse.json({ error: 'comedy_type is required' }, { status: 400 })
    }
    ...
    if (error || !data) {
      console.error('[comedy/save-session] insert failed:', error?.message)
      return NextResponse.json(
    ...
    console.error('[comedy/save-session] POST', msg)
...
export async function PATCH(req: Request) {
  try {
    ...
    const msg = e instanceof Error ? e.message : 'Unknown error'
    console.error('[comedy/save-session] PATCH', msg)
    return NextResponse.json({ error: msg }, { status: 500 })
  }
}
