...
import { parsePanelResponses, type PanelSessionResponse } from '@/lib/panel/session-types'
...
function parseResponsesBody(raw: unknown): PanelSessionResponse[] | null {
  if (!Array.isArray(raw) || raw.length < 1) return null
  const parsed = parsePanelResponses(raw)
...
export async function POST(req: Request) {
  try {
    ...
    const panel_type = typeof body.panel_type === 'string' ? body.panel_type.trim() : ''
    if (!panel_type) {
    ...
    if (error || !data) {
      console.error('[panel/save-session] insert failed:', error?.message)
      return NextResponse.json(
    ...
    console.error('[panel/save-session] POST', msg)
