 ...
 * inland). Shows next-arrival boards for a FIXED list of key stops (금정역·
 * 산본역 인근) instead of Jeju's GPS "nearby stations" search.
 *
            ...
            {notConfigured ? (
              <section className="rounded-2xl border-2 border-[#CBD5E1] bg-white p-5 shadow-sm">
                <p className="text-base font-semibold text-[#64748B]">
                  버스 정류소 정보가 아직 설정되지 않았어요 — cityCode·정류소(GUNPO_CITY_CODE / GUNPO_KEY_STOPS)가 TODO
                  상태예요.
                </p>
              </section>
            ) : (
              data.stops.map((stop) => (
                <section key={stop.nodeId} className="rounded-2xl border-2 border-[#CBD5E1] bg-white p-5 shadow-sm">
                  ...
                  {stop.error ? (
                    <p className="text-base font-semibold text-[#64748B]">{stop.error}</p>
                  ) : stop.rows.length === 0 ? (
                    <p className="text-base font-semibold text-[#64748B]">현재 도착 예정 버스 정보가 없어요.</p>
                  ) : (
