...
import { ChevronDown, ChevronUp, Loader2, PlayCircle, Search, Activity, AlertTriangle } from 'lucide-react'
...
import { getDiagnosticCategories } from '@/lib/gunpo/diagnostic-categories'
...
const DIAGNOSTIC_STAGE_LABEL: Partial<Record<StageKey, string>> = {
  start: '데이터 수집 중',
  search: '실시간 검색 중',
  status: '오늘의 현황 분석 중',
  issues: '시급 사안 진단 중',
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm font-semibold text-jeju-accent">
        {DIAGNOSTIC_STAGE_LABEL[stage] ?? '처리 중'}
      </p>
...
export function DiagnosticSection() {
  ...
  const categories = getDiagnosticCategories(toJejuCouncilMode(councilMode))
  ...
  const postWithRetry = useCallback(
    async (reqBody: Record<string, unknown>): Promise<DiagnosticApiResult | null> => {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
          const res = await fetch('/api/gunpo/diagnostic', {
            method: 'POST',
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Category buttons */}
        ...
        {running && <DiagnosticRunningBanner stage={stage} elapsed={elapsed} bannerRef={bannerRef} />}
