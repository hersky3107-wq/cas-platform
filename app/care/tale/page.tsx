...
/**
 * 이야기(TALE) — listen-first daily content for elderly users.
 *
 * A home with 4 big cards → tap one → its items load (cached instantly, or
 * generated once/day via /api/care/tale). Everything is meant to be LISTENED to:
...
type Kind = 'life' | 'health' | 'reminisce' | 'wisdom'
...
interface KindMeta {
  ...
  colorStrong: string
  bg: string
  /** whether this kind offers a "전체 읽어주기" (life stories & health talk) */
  readAll: boolean
}
...
export default function TalePage() {
  ...
  const openKind = useCallback(
    async (kind: Kind) => {
      ...
      setLoading(true)
      try {
        const res = await fetch(`/api/care/tale?kind=${kind}`, { method: 'GET' })
...
function TaleCard({
  ...
  // Life / health / reminisce — title + body (+ questions / note)
