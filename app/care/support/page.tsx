...
/**
 * 맞춤 지원찾기 — Welfare finder for elderly / digitally-vulnerable residents.
 *
 ...
 * Engine: lib/care/welfare.ts via /api/care/{interpret,welfare-match}.
 */
...
import { getResidence, DEFAULT_RESIDENCE, welfareRegionKey, type Residence } from '@/lib/care/residence'
...
export default function ResidentSupportPage() {
  ...
  const submit = useCallback(async () => {
    ...
    if (text) {
      try {
        const res = await fetch('/api/care/interpret', {
          method: 'POST',
    ...
    try {
      const res = await fetch('/api/care/welfare-match', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...profile,
          // Residence-driven region. Non-Jeju regions currently match only the
          // 'national' (central-government) welfare rows; regional welfare DATA
          // per province is the next step.
          region: welfareRegionKey(residence.sidoCode),
          sido: residence.sido,
    ...
    setPhase('results')
  }, [answers, freeText, stopSpeaking, residence])
