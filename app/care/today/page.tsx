...
/**
 * 오늘 날짜·날씨 — date/time/weather screen for elderly care mode.
 *
 * 지남력(orientation) support: very large date + day-of-week, live clock,
 * and the user's OWN city weather with a 7-day forecast (nationwide).
 *
 * Weather is driven by the residence lat/lng (lib/care/residence) via
 * lib/care/weather (getForecast). 요일/음력/시계 are nationwide as-is.
 *
...
import { getForecast, mapWeatherCode, type ForecastDay } from '@/lib/care/weather'
...
export default function TodayPage() {
  ...
  // ── Weather (single location = the user's residence) ───────────────────────
  ...
  const cityLabel = useMemo(() => shortLabel(residence), [residence])
  ...
  const speakToday = useCallback(() => {
    ...
    if (todayWx) {
      text += ` ${cityLabel} 날씨는 ${todayWx.label}, 최고 ${todayWx.tempMax}도, 최저 ${todayWx.tempMin}도입니다.`
    }
    speak(text)
  }, [now, lunarInfo, todayWx, cityLabel, speak])
    ...
    <div style={styles.root}>
      ...
      <main style={styles.frame}>
        ...
        <div style={styles.topBar}>
          <button type="button" className="td-ctrl" style={styles.ctrlBtn} onClick={() => { try { window.speechSynthesis?.cancel() } catch { /* */ } router.push('/care') }} aria-label="처음으로 돌아가기">
        ...
        {/* ── Weather ────────────────────────────────────────────────────── */}
        <section style={styles.wxSection} aria-label={`${cityLabel} 날씨 예보`}>
          <h2 style={styles.wxHeading}><span aria-hidden>🌦️</span> {cityLabel} 날씨 예보</h2>
          ...
          {!wxLoading && !wxError && days && (
            <>
              ...
              <div style={styles.forecastGrid} role="list" aria-label="날씨 예보">
                {days.slice(0, wxExpanded ? 7 : 3).map((day) => {
                  const wx = mapWeatherCode(day.weatherCode)
