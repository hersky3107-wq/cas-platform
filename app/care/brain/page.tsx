 ...
 *
 * Loads 6 code-graded multiple-choice questions from /api/care/brain
...
export default function BrainPage() {
  ...
  const start = useCallback(async () => {
    ...
    setPhase('loading')
    try {
      const res = await fetch(`/api/care/brain?level=${level}`, { method: 'GET' })
        ...
        {phase === 'intro' && (
          <>
            ...
            <button
              ...
              className="br-alt"
              style={styles.altBtn}
              onClick={() => { stopSpeaking(); router.push('/care/brain/memory') }}
