...
import type { TextInference } from "@/lib/oracle/tier2";
...
function isMajor(card: DrawnCard): boolean {
  if (card.arcana === "major") return true
  return typeof card.id === "number" && card.id >= 0 && card.id <= 21
}
...
export default function TarotSpreadChart({
  ...
  return (
    <div>
      ...
      {hasMajor && showJudgement ? (
        <AiJudgementNote inferences={inferences} pending="메이저 카드의 의미" />
      ) : null}
    </div>
  )
}
