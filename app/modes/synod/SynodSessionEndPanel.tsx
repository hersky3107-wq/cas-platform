...
import type { SynodLocale, SynodUiPack } from "@/lib/synod/ui-labels";
import { AI_COLORS, BRAND, DEBATERS } from "@/lib/synod/debaters";
...
type SynodSessionEndPanelProps = {
  sessionId: string;
  shareId: string | null;
  t: SynodUiPack;
  locale: SynodLocale;
...
export function SynodSessionEndPanel({
  sessionId,
  shareId,
  t,
  locale,
  ...
  const handleGoPublic = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/synod/go-public", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-4 sm:px-5">
      ...
      <div>
        ...
        {votedAi ? (
          <p className="mt-2 text-xs text-cyan-300/90">{t.voteThanks}</p>
        ) : null}
        {voteError ? (
          <p className="mt-2 text-xs text-amber-300/90">{voteError}</p>
        ) : null}
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
        {goPublicDone ? (
          <div className="text-sm">
            <p className="font-medium text-cyan-200">✅ {t.published}</p>
            {shareId ? (
          ...
          <>
            ...
            <div className="mt-3">
              <button
                ...
                className="inline-flex items-center justify-center rounded-full border border-cyan-400/50 bg-cyan-500/20 px-4 py-1.5 text-sm font-semibold text-cyan-300 transition hover:bg-cyan-500/30 disabled:cursor-not-allowed disabled:opacity-50"
              >
