...
/**
 * Shared AX COUNCIL brand band + urban/people segmented toggle
 * (cloned from components/motie/MotieModeBand.tsx).
 * Rendered at the top of every gunpo page (inside JejuThemeShell) so the mode
 * choice is always visible and consistent. Copy is mode-specific (placeholder).
 */

import { useJejuUi } from './useJejuUi'
import { useMotieMode, type GunpoMode } from './mode-context'

export function MotieModeBand() {
  const { t } = useJejuUi()
  const { mode, setMode } = useMotieMode()

  const options: { key: GunpoMode; label: string; sub: string }[] = [
    { key: 'urban', label: t.modeTrade, sub: t.modeTradeEn },
    { key: 'people', label: t.modeWarroom, sub: t.modeWarroomEn },
  ]
  const copy = mode === 'urban' ? t.tradeCopy : t.warroomCopy
